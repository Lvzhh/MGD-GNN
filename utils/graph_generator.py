...
def dependency_graph_generator(segment_char_span, dependency_head, max_seq_len, max_segment_len):
    adj = np.zeros((max_seq_len + max_segment_len, max_seq_len + max_segment_len), dtype=np.float32)
    for segment_offset, (char_start, char_end) in enumerate(segment_char_span):
        segment_idx = max_seq_len + segment_offset
        adj[char_start:char_end+1, segment_idx] = 1.0
    for offset, head_offset in enumerate(dependency_head):
        if head_offset == 0:
            continue
        segment_idx = max_seq_len + offset
        head_idx = max_seq_len + head_offset - 1
        adj[segment_idx, head_idx] = 1.0
    
    adj = adj + adj.T
    adj = adj + np.eye(max_seq_len + max_segment_len, dtype=np.float32)
    return adj
