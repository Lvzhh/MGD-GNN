...
class Data:
    ...
    def generate_instance(self, input_file, name, random_split=False, inst_type="entity"):
        texts, ids, cut_num = read_instance(input_file, self.gaz, self.char_alphabet, self.label_alphabet, self.gaz_alphabet, self.number_normalized, self.max_sentence_length, inst_type=inst_type)
        if name == "train":
            if random_split:
                random.seed(1)
